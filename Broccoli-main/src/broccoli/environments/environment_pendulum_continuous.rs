...
pub struct EnvironmentPendulumCont{
    pub angle: f64, //in radians
    pub angluar_velocity: f64,
    ...
    n_actions: usize,
    pub actions: Box<Vec<f64>>,
}

impl EnvironmentPendulumCont{
    pub fn new(actions: Vec<f64>) -> EnvironmentPendulumCont {
        EnvironmentPendulumCont {
            angle: 0.0,
...
impl Environment for EnvironmentPendulumCont {
    
    fn apply_action(&mut self, action: usize) {
        let u: f64 = if action >= self.n_actions {
            panic!();
        } else {
            self.actions[action]
        };
        ...
        self.angle = EnvironmentPendulumCont::normalise_angle(self.angle);
        // println!("after");
        // println!(
        //     "{} {} {}",
        //     self.angle.cos(),
    ...
    fn environment_info(&self) -> EnvironmentInfo {
        ...
        EnvironmentInfo::new(intervals, self.n_actions)
    }
}
...
mod tests {
    ...
    use super::EnvironmentPendulumCont;

    #[test]
    fn test() {
        let tmp = vec![-2.0, 2.0];
        let mut env = EnvironmentPendulumCont::new(tmp);
        env.reset(&[0.0, 0.0]);
    ...
    fn specific_state() {
        let tmp = vec![-2.0, 2.0];
        let mut env: EnvironmentPendulumCont = EnvironmentPendulumCont::new(tmp);
