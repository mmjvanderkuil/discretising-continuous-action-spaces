...
pub struct EnvironmentCartPoleCont {
    //state
    cart_position: f64,
    cart_velocity: f64,
    pole_angle: f64,
...
impl EnvironmentCartPoleCont {
    pub fn new(actions: Vec<f64>) -> EnvironmentCartPoleCont {
        let gravity = 9.8;
        let cart_mass = 1.0;
        let pole_mass = 0.1;
...
impl Environment for EnvironmentCartPoleCont {
    //taken from https://github.com/openai/gym/blob/master/gym/envs/classic_control/cartpole.py
    fn apply_action(&mut self, action: usize) {
        let force: f64 = if action >= self.n_actions {
            unreachable!();
        } else {
            self.actions[action]*self.force_magnitude
        };
...
//     use super::EnvironmentCartPoleCont;

//     #[test]
//     fn test() {
//         let mut env = EnvironmentCartPoleCont::new(&[0.0]);
