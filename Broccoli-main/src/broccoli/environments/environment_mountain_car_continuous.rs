...
pub struct EnvironmentMountainCarCont {
...
impl EnvironmentMountainCarCont {
    pub fn new(actions: Vec<f64>) -> EnvironmentMountainCarCont {
        EnvironmentMountainCarCont {
            state: vec![0.0, 0.0],
            force: 0.001,
            gravity: 0.0025,
    ...
    fn compute_velocity_change(&self, action: usize) -> f64 {
        let action_contribution: f64 = if action >= self.n_actions {
            unreachable!();
        } else {
            self.actions[action]*self.force
        };
...
impl Environment for EnvironmentMountainCarCont {
